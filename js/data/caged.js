/*
 * 5 форм CAGED — стандартная, общеизвестная аппликатура открытых аккордов
 * (C-A-G-E-D), не выдумка: форма C открытая = аккорд C, форма A = A, форма G = G,
 * форма E = E, форма D = D. R = тоника, 3/5 — соответствующие ступени.
 *
 * Каждая форма хранится в её "естественной" открытой позиции: string 1..6
 * (1 = самая тонкая/высокое E, 6 = самая толстая/низкое E), fret — абсолютный
 * номер лада В ЭТОЙ открытой позиции (0 = открытая струна).
 *
 * openRootPc — какую тонику (pitch class) даёт форма без баррэ (в открытой
 * позиции). Транспонизация в другую тональность — это баррэ, т.е. просто
 * сдвиг всех ладов на (target - openRootPc) полутонов, см. caged-screen.js
 * (transposeShape).
 *
 * Минорные формы CAGED пока не добавлены — только мажорные.
 */
